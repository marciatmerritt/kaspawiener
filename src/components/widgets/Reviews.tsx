        ...
        <div class="mx-auto max-w-screen-xl lg:py-16 lg:px-6">
          <div class="py-2 px-2 mx-auto bg-gray-200 dark:bg-gray-800 lg:py-16 lg:px-6">
            <div class="grid gap-8 lg:grid-cols-3">
              {reviews.map(({ title, text, name, role, image }, index) => (
                <figure
                  key={index}
                  class={`p-6 bg-gray-50 rounded dark:bg-gray-800 ${index === 0 ? 'border-8 border-gray-100' : ''}`}
                >
                  ...
                  <figcaption class="flex items-center space-x-3">
                    <img class="w-9 h-9 rounded-full" src={image} alt="profile picture" />
                    <div class="space-y-0.5 font-medium dark:text-white">
                      <div>{name}</div>
                      <div class="text-sm font-light text-gray-500 dark:text-gray-400">{role}</div>
                    </div>
                  </figcaption>
                </figure>
              ))}
